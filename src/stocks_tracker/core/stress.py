...
No hay modelo ni simulación: para cada escenario se toma lo que de verdad hizo
cada valor entre dos fechas reales y se aplica a los pesos de hoy. Un modelo
con betas normales dice siempre la misma mentira cómoda —en las caídas fuertes
las correlaciones se van hacia uno y las betas de tiempos tranquilos se quedan
cortas—, y se equivoca justo en la dirección que tranquiliza.

**Esto no es el peor caso.** El peor caso siempre es peor que lo peor que ha
pasado: en 2007 nadie tenía 2008 en su lista de escenarios. Sirve para ver
donde está concentrado el riesgo, no para poner un suelo a las pérdidas.

Y no es una predicción de nada. Tu cartera de hoy no existía entonces; lo que
...
class Fuente(StrEnum):
    """De donde sale el retorno que se aplica a una posición."""
...
ETIQUETA_FUENTE = {
    Fuente.PROPIA: "su propio histórico",
    Fuente.SECTOR: "su sector (el valor no cotizaba)",
    Fuente.MERCADO: "el índice (sin sector conocido)",
...
class Impacto:
    ...
    def cobertura(self) -> float:
        """Que fracción del dinero se estima con el histórico del propio valor.

        Es el número que dice si fiarse: con cobertura baja, esto describe el
        mercado de entonces más que tu cartera de ahora.
        """
        total = self.valor_total
        if total <= 0:
            return 0.0
    ...
    def peores(self) -> list[ImpactoPosicion]:
        """Las que más dinero se llevan, en euros y no en porcentaje.
...
def frase_peor(res: Impacto) -> str:
    ...
    Existe como función y no como f-string en la pantalla porque el caso raro
    —que el peor escenario sea una subida— sale mal con un `abs()` delante: la
    frase diría "habría caído un 10,9 %" sobre una cartera que habría ganado.
    Pasa con carteras defensivas y con cualquiera que lleve algo inverso.
    """
    nombre = res.escenario.nombre
    if res.retorno >= 0:
        return (
            f"Ni en el peor de los escenarios con datos (**{nombre}**) habría "
            f"perdido esta cartera: habría ganado un **{res.retorno:.1%}**. "
    ...
    return (
        ...
        f"hoy habría caído un **{abs(res.retorno):.1%}**, unos "
        f"**{abs(res.perdida):,.0f} EUR**. La pregunta útil no es si lo ves "
        "probable, sino si podrías aguantarlo sin vender."
    )
...
def impacto(escenario: Escenario, posiciones: list[dict],
    ...
    fuentes se prueban en orden de preferencia: el histórico del propio valor,
    el de su sector, y el del índice. Una posición sin ninguna de las tres se
    queda FUERA en vez de contar como que no se movió: suponer cero es la
    única hipótesis que garantiza equivocarse en la dirección tranquilizadora.
    """
...
class Diversificacion:
    ...
    Diez valores que se mueven todos igual son una apuesta, no diez. El número
    ...
    def se_pierde(self) -> float:
        """Cuanta diversificación desaparece cuando llega el momento malo."""
...
def _efectivas(pesos: np.ndarray, vols: np.ndarray, corr: np.ndarray) -> float:
    """Número efectivo de apuestas independientes.

    (suma de riesgos por separado)^2 / (riesgo de la cartera)^2. Vale N cuando
    no hay correlación y 1 cuando la correlación es total, que es justo lo que
...
def diversificacion(pesos: dict, correlaciones, volatilidades: dict | None = None,
    ...
    """Cuantas apuestas independientes hay hoy y cuantas quedarían en una caída.
    ...
    que se mide es co-movimiento— y evita quedarse sin número por falta de un
    dato secundario.
    """
