...
st.caption(
    ...
    "solo valor en operación corporativa y deja de describir al grupo."
)
...
st.subheader("Mapa de rotación")
if rotation.empty:
    st.caption(
        "Sin datos de rotación. Requiere los ETFs sectoriales del universo "
    ...
    with rot_right:
        ...
        st.caption(
            ...
            "índice. El vertical, si esa ventaja se acelera o se agota. La "
            "estela de puntos marca por donde ha pasado en las últimas semanas.\n\n"
...
group_choice = st.radio(
    ...
    format_func=lambda c: {"gics_sector": "Sector GICS",
                           "investment_type": "Tipo de inversión"}[c],
    horizontal=True,
)
treemap_data = da.get_treemap_data("TODOS", group_choice)
if treemap_data.empty:
    st.caption("Sin datos de capitalización suficientes.")
else:
    st.plotly_chart(
    ...
    st.caption(
        "Superficie proporcional a la capitalización, color según la variación "
        "del día. Agrupar por **tipo de inversión** es algo que el mapa de "
...
st.dataframe(
    ...
    column_config={
        **{
            ...
            for col in ["Día", "Semana", "Mes", "Trimestre", "12 meses"]
        },
...
if breadth.empty:
    st.caption("Sin serie de amplitud para este sector todavía.")
else:
    st.plotly_chart(
...
if tv_widgets.enabled():
    ...
    with tab_crypto:
        st.caption(
            "La cripto funciona como termómetro de apetito por riesgo: cuando "
