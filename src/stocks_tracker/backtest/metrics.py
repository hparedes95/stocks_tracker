...
def mean_p_value(returns, max_lag: int | None = None) -> float:
    """p bilateral del contraste de media usando el estadistico HAC."""
...
def benjamini_hochberg(p_values) -> np.ndarray:
    """Ajusta p-values por Benjamini-Hochberg (FDR), conservando posiciones."""
    ...
    restored[order] = np.clip(q_ranked, 0.0, 1.0)
    out[valid] = restored
    return out
...
def summarize_event(returns, benchmark_returns=None) -> EventMetrics:
    ...
    if benchmark_returns is None:
        ...
        mask = np.isfinite(raw) & np.isfinite(bench)
        values, bench = raw[mask], bench[mask]
    excess = values - bench
    return EventMetrics(
        n_obs=int(len(values)), hit_rate=float((values > 0).mean()),
        hit_rate_vs_benchmark=float((excess > 0).mean()), avg_return=float(values.mean()),
        median_return=float(np.median(values)), avg_excess=float(excess.mean()),
        std_return=float(values.std(ddof=1)) if len(values) > 1 else float("nan"),
        t_stat=hac_t_statistic(excess), p_value=mean_p_value(excess),
        best=float(values.max()), worst=float(values.min()), benchmark_avg=float(bench.mean()),
    )
